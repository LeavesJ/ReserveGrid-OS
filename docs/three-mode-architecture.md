...
- **v1.1.0:** Automatic mode degradation (inline→observe on verifier unreachable), extended channels + vardiff (PB-6; correction 2026-09-22: these two landed on 2026-04-22, after the `v1.1.0` tag), full per-IP rate limiter module, gateway Phase 1, policy model economic improvements. Shadow readiness gate (LicenseGate blocks access when shadow feed services absent). Dev deploy mode (purple badge, all features unlocked via dev passkey tier). Multi-pubkey license validation (ADR-001). `feed_adapter_url` dashboard health probe.
