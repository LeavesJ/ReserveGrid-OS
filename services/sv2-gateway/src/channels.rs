...
///
/// V1.0.0 uses a 4-byte monotonic counter as the extranonce prefix.
/// Each standard channel gets a unique prefix so that coinbase txids
/// (and therefore merkle roots) differ across channels.
...
impl ExtranonceAllocator {
    ...
    /// Allocate a 4-byte extranonce prefix. Returns `None` if exhausted.
    /// Once exhausted, all subsequent calls return `None` (no wraparound).
    pub fn allocate(&self) -> Option<[u8; 4]> {
        if self.exhausted.load(Ordering::Relaxed) {
            return None;
        }
...
/// State for a single standard mining channel.
#[derive(Debug)]
...
impl ChannelState {
    /// Create a new channel at open time.
    pub fn new(
        channel_id: u32,
        extranonce_prefix: [u8; 4],
        worker_id: String,
...
mod tests {
    ...
    fn connection_channels_limit_enforced() {
        ...
        mgr.open_channel(1, [0, 0, 0, 1], "w1".into(), None, default_target(), 0)
            .unwrap();
        mgr.open_channel(2, [0, 0, 0, 2], "w2".into(), None, default_target(), 0)
            .unwrap();
        let err = mgr
            .open_channel(3, [0, 0, 0, 3], "w3".into(), None, default_target(), 0)
            .unwrap_err();
    ...
    fn channel_state_prevhash_tracking() {
        let mut ch = ChannelState::new(1, [0, 0, 0, 1], "miner".into(), None, default_target(), 0);
    ...
    fn channel_state_has_rate_limiter() {
        let ch = ChannelState::new(1, [0, 0, 0, 1], "miner".into(), None, default_target(), 100);
