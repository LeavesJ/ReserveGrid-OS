...
- **rg-feed-adapter limits JSON-RPC calls per peer.** 1200 per peer per minute (`[adapter] rate_limit_per_minute`, `VELDRA_ADAPTER_RATE_LIMIT_PER_MINUTE`); `/health` is not limited. On loopback every caller shares one peer address, so the budget covers all of them together. A refused call gets 429 and `Retry-After`, and a limit of 0 or a value that does not parse stops startup.

### Changed

- **Operators: behind a reverse proxy, rg-dashboard now needs to be told where the proxy is.** The dashboard now allows 600 `/api` calls per client per minute (`[rate_limit] api_per_minute`) and 120 of `/api/health`; `/healthz` and the SPA are not limited, and a refused call gets 429 with `Retry-After`. Without `[rate_limit] trusted_proxies` and `client_ip_header` the client is the TCP peer, which behind a reverse proxy is the proxy, so every operator shares one budget: one client making about 10 requests a second through the proxy denies `/api`, `/api/auth/login` included, to everyone behind it. An existing deployment behind a proxy upgrades into this with no config error. The dashboard logs a warning once a minute, naming the connecting address, when requests carry `x-forwarded-for`, `cf-connecting-ip` or `forwarded` but are counted against that address. Trust a proxy address only when nothing else can connect from it: never Docker's bridge gateway while 8084 is published on every interface, as the shipped compose files publish it, because a trusted address can choose each client's dashboard budget and rg-auth login budget. See "Dashboard request limits" in `docs/deployment-runbook.md`. A limit of 0, or only one of the two trust keys, refuses to start.
